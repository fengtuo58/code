...
optlen：对于getsockopt()，作为入口参数时，选项值的最大长度。作为出口参数时，选项值的实际长度。对于
...
int main(int argc,char* argv[])
{
    ...
    signal(SIGPIPE,SIG_IGN);//the server close the connect and client sends message,the server will return a RST,if the client still sends msg ,server will send sigpipe ti client so that the client exit usually ,but you can keep the client alive by the signal(SIGPIPE,SIG_IGN);
    signal(SIGCHLD,SIG_IGN);
    signal(SIGINT,&on_sigint);
    listenfd=socket(AF_INET,SOCK_STREAM,0);
    setsockopt(listenfd,SOL_SOCKET,SO_REUSEADDR,&sock_op,sizeof(sock_op));//SO_REUSERADDR允许重用本地地址和端口　
